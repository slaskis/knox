...
var utils = require('./utils')
  , auth = require('./auth')
  , mime = require('./mime')
  , MultipartUploader = require('./multipart')
  , http = require('http')
  , https = require('https')
  , url = require('url')
...
Client.prototype.putStream = function(stream, filename, headers, fn){
  ...
  var dest = this.createWritableStream(filename, headers);
 ...
 * @api public
 */
Client.prototype.createWritableStream = function(filename, headers) {
  headers = utils.merge({
    'x-amz-acl': 'public-read'
  }, headers || {});

  return new MultipartUploader(this, filename, headers);
};
...
Client.prototype.copy = function(filename,source,headers){
  var bucket = this.endpoint;
  if( typeof source == "object" ){
    var src = Object.keys(source)[0];
    bucket = source[src].endpoint;
    source = src;
  }
  headers = utils.merge(headers||{},{
    'Content-Length': 0,
    'x-amz-copy-source': "/"+bucket.replace(".s3.amazonaws.com","")+source
  })
  return this.request('PUT', filename, headers);
  // TODO Check the response of req, if it's unauthorized, authorize read from source to the client
};
