...
function Multipart(client, filename, headers){
  ...
  console.log('multipart %s',this.filename)
  ...
  setup.on('response',function(res){
    if( res.statusCode !== 200 ){
      return buffer(res, function(body){      
        console.log(body)
        self.emit('error', new Error("Preparing MultiPart failed with "+res.statusCode));
      })
    }
    ...
    buffer(res, function(body){
      ...
      console.log('setup (%d) %d chunks',res.statusCode,self.buffer.length)
...
Multipart.prototype = {
  ...
  uploadPart: function(){
    ...
    req.on('response',function(res){
      if( res.statusCode !== 200 ){
        return buffer(res, function(body){
          console.error('uploadPart',body)
          self.emit('error', new Error('Failed to upload part:'+pnr))
        })
      }
    ...
    console.log('uploadPart %d bytes',len)
  ...
  completeEmptyFile: function(){
    ...
    console.log('complete empty file')
