...
Multipart.prototype = {
  ...
  write: function(chunk,enc){
    if( !Buffer.isBuffer(chunk) )
      chunk = new Buffer(chunk.toString(),enc);
    ...
    if( this.chunkBufferLength >= MIN_CHUNK_SIZE ) {
      ...
      // we have a full upload buffer, apply backpressure.
    ...
    // waiting for s3 to initiate upload
    // try again on 'drain'
    if( this.paused )
      return this.once('drain',this.end.bind(this));

    // no more writing, just send what's left
    if( this.writable ){
      this.writable = false;
